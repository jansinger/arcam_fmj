...
def _make_reader(data: bytes) -> asyncio.StreamReader:
    """Create a StreamReader fed with the given bytes."""
...
async def test_read_response_valid_packet():
    ...
    reader = _make_reader(packet.to_bytes())
...
async def test_read_response_skips_invalid_packet():
...
async def test_read_response_skips_null_packet():
...
async def test_read_response_connection_failed_on_incomplete():
...
async def test_read_command_valid_packet():
...
async def test_read_command_amx_duet():
    """Test read_command parses an AMX Duet request."""
    reader = _make_reader(b"AMX\r")
    result = await read_command(reader)
    assert isinstance(result, AmxDuetRequest)


async def test_read_command_eof():
...
async def test_read_command_skips_invalid_packet():
...
async def test_read_response_amx_with_caret_header():
    ...
    reader = _make_reader(amx_data)
    result = await read_response(reader)
    assert isinstance(result, AmxDuetResponse)
    assert result.device_model == "AVR30"


async def test_read_response_amx_with_plain_header():
...
async def test_read_response_amx_caret_invalid_header():
...
async def test_read_response_amx_plain_a_invalid_header():
...
async def test_read_command_amx_with_caret_header():
...
async def test_write_packet_command():
    ...
    # Use a real StreamReader/Writer pair via socket pair is overkill;
