"""Enums, version sets, and mapping tables for the Arcam protocol."""
...
class IntOrTypeEnum(enum.IntEnum):
    ...
    def __new__(cls, value: int, version: set[str] | None = None, flags=EnumFlags(0)):  # noqa: B008
        obj = int.__new__(cls, value)
        obj._value_ = value
        obj.version = version
        obj.flags = flags
...
class SourceCodes(enum.Enum):
    ...
    def from_bytes(cls, data: bytes, model: ApiModel, zn: int) -> "SourceCodes":
        try:
            ...
            raise ValueError(f"Unknown source map for model {model} and zone {zn}") from err
        for key, value in table.items():
            if value == data:
                return key
        raise ValueError(f"Unknown source code for model {model} and zone {zn} and value {data!r}")
    ...
    def to_bytes(self, model: ApiModel, zn: int):
        ...
        if data := table.get(self):
            return data
        raise ValueError(f"Unknown byte code for model {model} and zone {zn} and value {self}")
...
HDA_SOURCE_MAPPING = {
    ...
    SourceCodes.DAB: bytes([0x0C]),
    SourceCodes.NET: bytes([0x0E]),
    SourceCodes.USB: bytes([0x0F]),  # Not in official protocol doc, but present on some HDA devices
    SourceCodes.STB: bytes([0x10]),
    SourceCodes.GAME: bytes([0x11]),
...
SA_SOURCE_MAPPING = {
    ...
    # TODO: verify USB against Arcam SA series docs (was duplicate 0x0B)
    SourceCodes.USB: bytes([0x0C]),
